...
def detect(img_raw):
    ...
    # img_raw = cv2.resize(img, (new_width, new_height), interpolation=cv2.INTER_AREA)
    im_height, im_width, _ = img.shape
...
if __name__ == '__main__':
    ...
    if args.fourcc ==0:
        ...
        os.makedirs('test_STN_end2end_LP', exist_ok = True)
        index = 0
        for image in os.listdir('test_STN_end2end'):
            img = cv2.imread(f"test_STN_end2end/{image}")
            img_raw = detect(img)

            # save image
            if args.save_image:
                name = f'test_STN_end2end_LP/out_{index}' + '.jpg'
