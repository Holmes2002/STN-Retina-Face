...
parser.add_argument('--freeze_box_head', default=False, help='Location to save checkpoint models')
...
if args.freeze_box_head:
    for layer in net.BboxHead:
        for param in layer.parameters():
            param.requires_grad = False
for layer in net.LandmarkHead:
        for param in layer.parameters():
            param.requires_grad = False

params      = [p for name, p in net.named_parameters() if p.requires_grad]
params_name = [name for name, p in net.named_parameters() if p.requires_grad]
print("[INFOR] Parameters: ", params_name)
...
def train():
    ...
    epoch_size = math.ceil(len(dataset) / batch_size)-1
    ...
    for iteration in range(start_iter, max_iter):
        ...
        images = images.cuda()
        device_targets = [anno.cuda() for anno in targets]
        ...
        # backprop
        optimizer.zero_grad()
        loss_l, loss_c, loss_landm = criterion(out, priors, device_targets, epoch, images)
        # loss = loss_c + loss_landm + cfg['loc_weight'] * loss_l
        loss = loss_c  + cfg['loc_weight'] * loss_l
        loss.backward()
        optimizer.step()
        load_t1 = time.time()
